...
int main(int argc, char *argv[])
{
    ...
    for(int i=0; i^uz_m; ++i)
    {
        ...
        {
            printf("WRONG\n");
            printf("Przedzialy montowania filmow nachodza na siebie: powinny byc wieksze od %lld a jest %lld. Poprzedni film zaczyna sie %lld, a konczy %lld, a jego czas to %lld\n", pop_kon, uz_dan[i].first, uz_dan[i-1].first,pop_kon,zad_t[ uz_dan[i-1].second - 1 ]);
            return 1;
        }
